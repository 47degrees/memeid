...
class UUIDSpec extends Specification with ScalaCheck {
  ...
  "uuid.asV* methods" should {
    ...
    "uuid.asV1 should return optional with uuid only if class is UUID.V1" >> {
      ...
      ((uuid.asV1 must be)
        .equalTo(Optional.of[UUID.V1](uuid)))
        .and((uuid.asV2 must be).equalTo(Optional.empty[UUID.V2]))
        .and((uuid.asV3 must be).equalTo(Optional.empty[UUID.V3]))
        .and((uuid.asV4 must be).equalTo(Optional.empty[UUID.V4]))
        .and((uuid.asV5 must be).equalTo(Optional.empty[UUID.V5]))
    }

    "uuid.asV2 should return optional with uuid only if class is UUID.V2" in prop { uuid: UUID.V2 =>
      ((uuid.asV1 must be)
        .equalTo(Optional.empty[UUID.V1]))
        .and((uuid.asV2 must be).equalTo(Optional.of[UUID.V2](uuid)))
        .and((uuid.asV3 must be).equalTo(Optional.empty[UUID.V3]))
        .and((uuid.asV4 must be).equalTo(Optional.empty[UUID.V4]))
        .and((uuid.asV5 must be).equalTo(Optional.empty[UUID.V5]))
    }
    ...
    "uuid.asV3 should return optional with uuid only if class is UUID.V3" >> {
      ...
      ((uuid.asV1 must be)
        .equalTo(Optional.empty[UUID.V1]))
        .and((uuid.asV2 must be).equalTo(Optional.empty[UUID.V2]))
        .and((uuid.asV3 must be).equalTo(Optional.of[UUID.V3](uuid)))
        .and((uuid.asV4 must be).equalTo(Optional.empty[UUID.V4]))
        .and((uuid.asV5 must be).equalTo(Optional.empty[UUID.V5]))
    }
    ...
    "uuid.asV4 should return optional with uuid only if class is UUID.V4" >> {
      ...
      ((uuid.asV1 must be)
        ...
        .and((uuid.asV2 must be).equalTo(Optional.empty[UUID.V2]))
        .and((uuid.asV3 must be).equalTo(Optional.empty[UUID.V3]))
        .and((uuid.asV4 must be).equalTo(Optional.of[UUID.V4](uuid)))
        .and((uuid.asV5 must be).equalTo(Optional.empty[UUID.V5]))
    }
    ...
    "uuid.asV5 should return optional with uuid only if class is UUID.V5" >> {
      ...
      ((uuid.asV1 must be)
        ...
        .and((uuid.asV3 must be).equalTo(Optional.empty[UUID.V3]))
        .and((uuid.asV4 must be).equalTo(Optional.empty[UUID.V4]))
        .and((uuid.asV5 must be).equalTo(Optional.of[UUID.V5](uuid)))
    }
  ...
  "uuid.isV* methods" should {
    ...
    "uuid.isV1 should return true only if class is UUID.V1" >> {
      ...
      (uuid.isNil must beFalse)
        .and(uuid.isV1 must beTrue)
        .and(uuid.isV2 must beFalse)
        .and(uuid.isV3 must beFalse)
        .and(uuid.isV4 must beFalse)
        .and(uuid.isV5 must beFalse)
    }

    "uuid.isV2 should return true only if class is UUID.V2" in prop { uuid: UUID.V2 =>
      (uuid.isNil must beFalse)
        .and(uuid.isV1 must beFalse)
        .and(uuid.isV2 must beTrue)
        .and(uuid.isV3 must beFalse)
        .and(uuid.isV4 must beFalse)
        .and(uuid.isV5 must beFalse)
    }
    ...
    "uuid.isV5 should return true only if class is UUID.V5" >> {
      ...
      (uuid.isNil must beFalse)
        ...
        .and(uuid.isV3 must beFalse)
        .and(uuid.isV4 must beFalse)
        .and(uuid.isV5 must beTrue)
    }

    "uuid.isNil should return true only if uuid is NIL uuid" >> {
      (UUID.NIL.isNil must beTrue)
        .and(UUID.NIL.isV1 must beFalse)
        .and(UUID.NIL.isV2 must beFalse)
        .and(UUID.NIL.isV3 must beFalse)
        .and(UUID.NIL.isV4 must beFalse)
        .and(UUID.NIL.isV5 must beFalse)
    }
  ...
  "UUID.NIL" should {
    ...
    "have all 128 bits to 0" in {
      ((UUID.NIL.getMostSignificantBits must be)
        .equalTo(0L))
        .and((UUID.NIL.getLeastSignificantBits must be).equalTo(0L))
    }
